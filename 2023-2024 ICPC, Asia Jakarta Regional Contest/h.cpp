...
Int jahja(int now,int rem)
{
  ...
  if (now == 26) return 1;
  Int &ret = dp[now][rem];
  if (dah[now][rem]) return ret;
  dah[now][rem] = 1;
 
  ret = 0;
  if (rem > adaa[now]) return ret = jahja(now,adaa[now]);
  if (rem == 0)
  {
    ret = jahja(now+1,adab[now+1] - adaa[now]) * binom.choose(sisa[now],adaa[now]);
  }
  else
  {
    ret = jahja(now,rem-1);
    ret += jahja(now+1,adab[now+1] - (adaa[now] - rem)) * binom.choose(sisa[now],adaa[now]) * binom.choose(adaa[now],rem);
  }
  // printf("%d %d %d\n",now,rem,ret.get());
  return ret;
}
...
int main()
{
  ...
  REP(i,n) ++adaa[a[i]-'A'];
  REP(i,m) ++adab[b[i]-'A'];
  binom = BinomCoeff(2 * m);
  sisa[26] = 0;
  FORD(i,25,0) sisa[i] = sisa[i+1] + adaa[i];
  Int risan = jahja(0,adab[0]);
  printf("%d\n",risan.get());
  return 0;
}