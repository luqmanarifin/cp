...
long long kiribawah(long long r, long long c) {
  ...
  ret += calc(rumus, 1, m);
  // kiri
  rumus = arith(polynom({1, mod-4,4}), polynom({-1, 1})); 
  ret += calc(rumus, 1, m);
  if (r > c) {
...
int main() {
  ...
  cerr << kiriatas(a, b) << " " << kananatas(c, d) << endl;
  long long mid = n/2 + 1;
  // cerr << mid << endl;
  long long ans = 0;
  if (a <= mid) {
  ...
  if (a <= mid && mid <= c) {
    if (b < mid && mid < d) {
      polynom rumus = arith(polynom({1, -3, 4}), polynom({0, 1}));
      ans += rumus.get(mid-b);
      rumus = arith(polynom({1, 1, 4}), polynom({0, 1}));
      ans += rumus.get(d-mid);
    }
    else if (b < mid) {
      polynom rumus = arith(polynom({1, -3, 4}), polynom({0, 1}));
      ans += rumus.get(mid-b) - rumus.get(mid-d-1);
    }
    else if (mid < d) {
      polynom rumus = arith(polynom({1, 1, 4}), polynom({0, 1}));
      ans += rumus.get(d-mid) - rumus.get(b-mid-1);   
    }
  }
  if (b <= mid && mid <= d) {
    if (a < mid && mid < c) {
      ...
      polynom rumus = arith(polynom({1, -1, 4}), polynom({0, 1}));
      ans += rumus.get(mid-a) - rumus.get(mid-c-1);
    }
    else if (mid < c) {
      polynom rumus = arith(polynom({1, 1, 4}), polynom({0, 1}));
      ans += rumus.get(c-mid) - rumus.get(a-mid-1);   
    }
  }
  ans %= mod;
  if (ans < 0)
