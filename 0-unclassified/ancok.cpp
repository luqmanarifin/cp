...
// Build lower hull
for (int i = 0; i < n; i++) {
while (k >= 2 && cross(H[k-2], H[k-1], P[i]) < 0) k--; // this prob says in order walking counter-clockwise so <0 else use <=0
H[k++] = P[i];
}
// Build upper hull
for (int i = n-2, t = k+1; i >= 0; i--) {
while (k >= t && cross(H[k-2], H[k-1], P[i]) < 0) k--; // this prob says in order walking counter-clockwise so <0 else use <=0
H[k++] = P[i];
}
H.resize(k);
return H;
...
printf("%d\n",sz);
REP(i,sz) cout << hullPoints[i].x << " " << hullPoints[i].y << endl;
}
// system("pause");
return 0;
}
