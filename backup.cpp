...
// keliru - fail dist method

using namespace std;
 
const long double eps = 1e-13;
const int iter_arc = 100;
const int iter_rad = 300;
...
long double dist(long double a, long double b, long double c, long double d) {
  long double dx = labs(a - c);
  long double dy = labs(b - d);
  long double mn = lmin(dx, dy);
  long double mx = lmax(dx, dy);
  
    return sqrt(mn * mn + mx * mx);
  
  long double ret = mx * 1007 + mn * 441;
  if(mx < (mn * 16)) {
    ret -= (mx * 40);
  }
  return (ret + 512) / 1024;
  ...
  while(j > eps && i < num.size()) {
    ...
    for(int it = 0; it < iter_arc; it++) {
      ...
      if(dir == 0) {
        ...
        long double dist2 = dist(c * cos(mida), c * sin(mida), x2, y2) - rad[2];
        long double distbef = dist(c * cos(mida), c * sin(mida), x, y) - j;
        //printf("found %.18lf %.18lf %.18lf\n", (double) dist2, (double) distbef, (double) midj);
        now = (lj + rj) / 2;
        break;
      }
    }
