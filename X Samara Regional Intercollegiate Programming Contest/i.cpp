...
// apply A[a] = k
void apply(mat A, corners a, datatype k)
{
    int i, j;
    for (i = a.ra; i < a.rb; i++)
        for (j = a.ca; j < a.cb; j++)
...
void add(mat A, mat B, mat C, corners a, corners b, corners c)
{
    ...
    for (i = 0; i < rd; i++)
    {
        for (j = 0; j < cd; j++)
        {
            long long& now = C[i + c.ra][j + c.ca];
            now = A[i + a.ra][j + a.ca] + B[i + b.ra][j + b.ca];
            if (now >= mod) now -= mod;
        }
    }
}
...
#define ST0 apply(S,p,0); apply(T,p,0)
